...
def generate_hierachical_tree(K, min_children=1, random_strategy=np.random.randn):
    G = artificial_loader.generate_hierarchical_tree(K, min_children=min_children, random_strategy=random_strategy)
    return tree_utils.tree_graph_builder(G)


def vizualize_samples(dataloader, tree, selected_layers, n_viz=4, autofill=False):
...
def generate_pln_data(pln_layers, n_samples, K, selected_layers, X_base, tree):
    n_samples = 10_000
    ...
    for layer, K_l in enumerate(K):
        ...
        X_pln_numpy, Z_pln_numpy = pln_lib.sample(pln, n_samples)
        Z_pln_enc_numpy = pln_lib.encode(pln)
        X_pln_enc_numpy = pln_lib.decode(Z_pln_enc_numpy)
...
def generate_multiple_models(models, n_samples, X_base):
    generations = []
    encodings = []
    for estimator, losses in models:
        X_plntree, Z_plntree, O_plntree = estimator.sample(n_samples)
        Z_plntree_enc, O_plntree_enc = estimator.encode(X_base)
        X_plntree_enc = estimator.decode(Z_plntree_enc, O_plntree_enc)
